...
class RegisteredGuest(Guest):
    ...
    login_id: Mapped[int] = mapped_column(ForeignKey("logins.id"))
...
class Room(Base):
    ...
    type: Mapped[str] = mapped_column(nullable=True)  # e.g. "family room", "single room", etc.
    max_guests: Mapped[int] = mapped_column("max_guests")
    description: Mapped[str] = mapped_column(nullable=True)  # e.g. "Room with sea view"
    amenities: Mapped[str] = mapped_column(nullable=True)
    price: Mapped[float] = mapped_column("price")
    is_available: Mapped[bool] = mapped_column("is_available", default=True)
    bookings: Mapped[List[Booking]] = relationship("Booking", back_populates="room")

    def __repr__(self) -> str:
        return f"Room(hotel={self.hotel!r}, room_number={self.number!r}, type={self.type!r}, description={self.description!r}, amenities={self.amenities!r}, price={self.price!r}, is_available={self.is_available!r})"
